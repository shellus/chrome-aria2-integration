...
(function(){
	var endAddr;
    var rpcpath = settings.get('rpcpath');
    var rpcuser = settings.get('rpcuser');
    var rpctoken = settings.get('rpctoken');
	if (rpcuser) {
		endAddr = rpcpath.replace(/(\:\/\/)/g, '$1' + rpcuser + ':' + rpctoken + '@');
		window.rpct = '';
	} else {
		endAddr = rpcpath;
		if (rpctoken) {
			window.rpct = 'token:' + rpctoken;
		} else {
			window.rpct = '';
		}
	}
